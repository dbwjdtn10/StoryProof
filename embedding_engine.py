    ...
    def _init_pinecone(self):
        ...
        try:
            ...
            if self.index_name not in available_indexes:
                ...
                print(f"[Debug] Configured Index: {self.index_name}")
                print(f"[Debug] API Key: {self.pinecone_api_key[:10]}..." if self.pinecone_api_key else "[Debug] API Key: None")
            else:
                self.index = self.pc.Index(self.index_name)
                print(f"[Success] Pinecone 인덱스 연결: {self.index_name}")
                print(f"[Debug] Connected to {self.index_name}")
