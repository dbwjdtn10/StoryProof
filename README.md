...
전처리 (사전 작업): 소설 전체 내용을 '씬(Scene)'이나 일정 단위로 쪼개서 임베딩(숫자 데이터)으로 변환한 뒤 Pinecone에 저장해 둡니다.

검색 (Retrieval): 사용자가 "다음 내용은 어떻게 될까?"라고 물으면, 백엔드는 그 질문과 가장 유사하거나 맥락이 이어지는 일부 장면들만 Pinecone에서 찾아냅니다.

답변 생성 (Generation): 찾아낸 **특정 장면들(Context)**과 사용자의 질문을 Gemini AI에게 전달하여 "이 내용을 바탕으로 예측해줘"라고 요청합니다.
