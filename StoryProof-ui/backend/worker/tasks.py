...
def detect_inconsistency_task(self, novel_id: int, text_fragment: str, chapter_id: Optional[int] = None, model: Optional[str] = None):
    try:
        agent = StoryConsistencyAgent(api_key=settings.GOOGLE_API_KEY)
        # 동기 환경(Celery)에서 비동기 함수 실행
        result = asyncio.run(agent.check_consistency(novel_id, text_fragment, chapter_id, model=model))
        return result
