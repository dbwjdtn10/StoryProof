...
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y python3-pip python3-venv python3-dev libpq-dev postgresql postgresql-contrib redis-server nginx supervisor git build-essential
...
directory=$APP_DIR
command=$VENV_DIR/bin/gunicorn -w 1 -k uvicorn.workers.UvicornWorker backend.main:app --bind 0.0.0.0:8000
user=$USER
autostart=true
autorestart=true
...
directory=$APP_DIR
command=$VENV_DIR/bin/celery -A backend.worker.celery_app worker --loglevel=info
user=$USER
...
# Grant Nginx permissions to access the home directory
