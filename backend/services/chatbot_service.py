...
class ChatbotService:
    ...
    def _build_rag_prompt(self, question: str, context: str, bible: str = "") -> str:
        ...
        학습 데이터 사용 방지: 추출 시스템으로 프레이밍, 절대 규칙 최상단 배치.
        """
        bible_block = f"\n[소설 바이블 — 등장인물/관계/사건]:\n{bible}\n" if bible else ""

        return f"""당신은 소설 문맥 추출 시스템입니다. 아래 [소설 문맥]에서 정보를 찾아 그대로 전달하는 것이 유일한 역할입니다.

━━━ 절대 규칙 (위반 불가) ━━━
1. [소설 문맥]과 [소설 바이블]에 명시된 내용만 사용할 수 있습니다.
2. 사전 학습 지식 사용 금지 — 유명 소설·동화·드라마·역사·실존 인물 등 어떤 외부 지식도 답변에 포함하지 마세요. 등장인물 이름이나 배경이 유명 작품과 같아도 예외 없이 적용됩니다.
3. [소설 문맥]에 관련 내용이 없으면 → "소설에서 해당 내용을 찾을 수 없습니다." 한 문장만 쓰고 끝내세요. 추측·보완·유추 설명을 추가하지 마세요.
4. [소설 문맥]에 근거가 있는 내용만 답변에 포함하고, 출처 표시([Context N])는 생략하세요.
━━━━━━━━━━━━━━━━━━━━━━━━
{bible_block}
[소설 문맥]:
...
위 [소설 문맥]에서 근거를 찾아 답변하세요. 근거가 없으면 "소설에서 해당 내용을 찾을 수 없습니다."
