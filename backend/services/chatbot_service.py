...
class ChatbotService:
    ...
    def _build_rag_prompt(self, question: str, context: str, bible: str = "") -> str:
        ...
        Adaptive format: 컨텍스트가 풍부하면 상세 답변, 부족하면 간결 답변.
        Reasoning steps: 내부 추론 과정을 통해 정확성 향상.
        """
        bible_block = f"\n\n[소설 바이블 (등장인물/관계/사건)]:\n{bible}" if bible else ""

        # 컨텍스트 풍부도에 따라 포맷 조정
        context_count = context.count('[Context ')
        if context_count >= 3:
            format_instruction = """**핵심 요약**
(질문에 대한 핵심 답변 1~2문장)

**상세 설명**
(소설 문맥의 구체적 내용과 근거를 서술. 관련 장면이나 대사가 있으면 자연스럽게 인용)

**관련 장면**
(답변의 근거가 된 주요 장면을 1~2개 간략히 언급)"""
        else:
            format_instruction = """**핵심 요약**
...
[핵심 규칙]
1. **[소설 문맥]과 [소설 바이블]에 있는 정보만 사용하세요.** 사전 학습된 지식, 원작 소설, 동화, 실존 인물에 관한 외부 지식은 절대 사용하지 마세요.
2. **이 소설은 독자적인 세계관을 가진 작품입니다.** 제목이나 등장인물 이름이 유명 작품과 비슷하더라도, 반드시 [소설 문맥]에 나온 내용만 기반으로 답변하세요.
3. **여러 장면의 정보를 종합하여 답변할 수 있습니다.** 단, 소설에 명시된 내용의 연결과 종합만 허용하며, 소설에 없는 사실을 지어내면 안 됩니다.
4. **[소설 문맥]에 관련 정보가 전혀 없으면** "소설에서 해당 내용을 찾을 수 없습니다."로 시작하세요.
5. **출처 표시([Context N] 등)는 포함하지 마세요.**

[추론 과정 — 답변 전에 내부적으로 수행하라]
1단계: 질문의 의도를 파악하라 (사실 확인 / 인과관계 / 비교 / 추론 / 요약).
2단계: [소설 문맥]에서 질문과 직접 관련된 정보를 모두 찾아라.
3단계: [소설 바이블]에서 보충할 정보가 있는지 확인하라.
4단계: 찾은 정보들을 종합하여 답변을 구성하라. 정보 간 모순이 있으면 명시.
5단계: 답변에 [소설 문맥] 외부 지식이 포함되었는지 검토하고, 포함되었다면 제거하라.
...
[답변 형식]
{format_instruction}{bible_block}
