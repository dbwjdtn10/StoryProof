  ...
  "results": [
    {
      ...
      "severity": "치명적" 또는 "주의" 또는 "참고",
      "quote": "▶ [검토 문장]에서 직접 발췌한 문제 구절 (원문 그대로 인용)",
      "evidence": "▶ [기존 설정] 또는 [바이블 요약]에서 발췌한 근거 문장 (원문 그대로 인용)",
      "description": "기존 설정(evidence)과 검토 문장(quote)을 대비하여 모순·당혹감을 구체적으로 분석",
      "suggestion": "기존 설정을 유지하면서 수정 가능한 대체 문장 또는 삽입할 복선 문구 (구체적 문구 필수)"
    }
  ]
}
...
[채점 보정 규칙]
- 90~100점: 출판된 문학 작품 수준의 완성도. 이 점수대에는 반드시 1개 이상의 weakness를 기록.
- 70~89점: 전반적으로 양호하나 개선점 존재. 대부분의 텍스트가 이 범위.
- 50~69점: 구조적 문제가 다수. 가독성은 있으나 서사 완성도 부족.
- 50점 미만: 심각한 구조적 결함.
- 만점(100)은 부여하지 마라. 어떤 텍스트든 최소 1가지 개선점은 존재한다.
...
- 결과가 없는 항목은 빈 배열 [].
- 문제 없는데 억지로 만들지 마라. 하지만 개선 제안(suggestions)은 항상 1개 이상 포함.
...
[채점 보정 규칙]
- 90~100점: 출판된 문학 작품 수준. 반드시 1개 이상의 weakness 기록.
- 70~89점: 양호하나 개선점 존재. 대부분의 텍스트가 이 범위.
- 50~69점: 문체 문제 다수. 가독성은 있으나 완성도 부족.
- 50점 미만: 심각한 문체 결함.
- 만점(100)은 부여하지 마라. 어떤 텍스트든 최소 1가지 개선점은 존재한다.
