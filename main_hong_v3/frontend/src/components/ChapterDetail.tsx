...
import { PredictionSidebar, Message } from './PredictionSidebar';
...
이건 그다지 놀라운 일이 아니었어요. 앨리스는 토끼가 혼잣말로 "이런, 이런! 늦겠어!"라고 말하는 걸 들었을 때도 그게 그렇게 이상하다고 생각하지 않았어요. (나중에 생각해 보니 이상하게 여겼어야 했지만, 그때는 모든 게 너무나 자연스러워 보였거든요.) 하지만 토끼가 실제로 조끼 주머니에서 시계를 꺼내 보더니 서둘러 달려가는 걸 보자, 앨리스는 벌떡 일어났어요. 조끼 주머니가 달린 토끼도, 거기서 꺼낼 시계를 가진 토끼도 본 적이 없다는 생각이 번뜩 스쳤거든요. 호기심에 불타 앨리스는 들판을 가로질러 토끼를 쫓아갔고, 토끼가 울타리 밑 큰 토끼 굴로 뛰어들어 가는 걸 보았어요.`);
    ...
    const handlePredictionSidebarOpen = () => {
        ...
        // If chat is empty, maybe show a welcome message or just empty state
    };
    ...
    const handleSendMessage = async (inputMessage: string) => {
        ...
        try {
            const response = await fetch('http://localhost:8000/api/v1/analysis/prediction', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ novel_id: novelId, text: inputMessage })
            });

            if (!response.ok) throw new Error("Failed to start prediction task");
    ...
    // Legacy handler for the modal (which is now just a button trigger effectively)
    const handlePredictStoryTrigger = () => {
        // Instead of opening modal, directly open sidebar
    ...
    return (
        <div className="chapter-detail-container">
            ...
            {/* Modal removed/disabled as we moved to Sidebar directly */}
            {/* <PredictionModal ... /> */}
        </div>
    );
}
