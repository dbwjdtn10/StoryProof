...
답변:"""
        ...
        target_model = model if model else settings.GEMINI_CHAT_MODEL
        print(f"DEBUG: Using model {target_model} for chat answer generation")
    ...
    def augment_query(self, question: str, model: Optional[str] = None) -> str:
        """
...
Output:"""
        ...
        try:
            ...
            print(f"🧬 Query Augmented using {target_model}: '{question}' -> '{augmented}'")
            return augmented
        except Exception as e:
            print(f"⚠️ Query Augmentation Failed using {target_model}: {e}")
            return question
    ...
    def ask(
        ...
        if not top_chunks:
            ...
            augmented_query = self.augment_query(question, model=model)
        ...
        # 4. LLM으로 답변 생성
        answer = self.generate_answer(question, context, model=model)
