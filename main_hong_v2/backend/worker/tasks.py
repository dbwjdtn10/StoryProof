...
from backend.db.models import Novel, Chapter, User, StoryboardStatus, Analysis, AnalysisType, AnalysisStatus
...
def detect_inconsistency_task(self, novel_id: int, text_fragment: str):
    try:
        agent = StoryConsistencyAgent(api_key=settings.GOOGLE_API_KEY)
        # 동기 환경(Celery)에서 동기 함수 실행 (asyncio.run 불필요)
...
def predict_story_task(self, novel_id: int, user_input: str):
    try:
        agent = StoryConsistencyAgent(api_key=settings.GOOGLE_API_KEY)
        result = asyncio.run(agent.predict_story(novel_id, user_input))
        return result
    except Exception as exc:
        raise self.retry(exc=exc, countdown=30)
...
# Note: This function doesn't have a @celery_app.task decorator in the original file. 
# If it is intended to be a Celery task, it should be decorated. 
# Assuming it is called synchronously or by another mechanism for now, based on previous file content.
# However, the docstring says "background task". 
# Given I am fixing "predict_story_task", I will leave this as is to avoid breaking existing behavior, 
# but simply clean up the file structure.
...
# ===== (Stub functions / Placeholders from original file) =====
