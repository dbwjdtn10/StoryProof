...
- [x] **검색 품질 개선 — 코드 수정** (E2E 데모에서 발견, 2026-07-11 / 수정 2026-07-13):
  ...
  - [x] **재인덱싱 완료** (2026-07-13): Docker Postgres 볼륨이 `create_all`
    기반이라 Alembic과 어긋나 있던 것을 발견 — 파일럿 전이라 실 고객 데이터가
    없어(2026-07-11 E2E 데모용 테스트 소설뿐) 볼륨을 초기화하고
    `alembic stamp head`로 재정렬. 프리픽스 없이 임베딩됐던 구 Pinecone
    벡터(2건, 전부 테스트 데이터)는 사용자 승인 하에 전체 삭제. 이후 실
    Postgres+Celery+Gemini+Pinecone 전체 스택으로 새 원고를 처음부터
