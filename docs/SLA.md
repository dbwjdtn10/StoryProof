...
> 측정일: 2026-07-10 (API 계층/하네스), 2026-07-13 (프로덕션 스택 추가) · 도구: `scripts/loadtest/` (재현 방법은 문서 하단)
...
프로덕션 스택(Docker + PostgreSQL + 실제 LLM) 재측정은 2026-07-13에 quick 모드
(동시성 10만)로 완료됨 — 결과는 [2-1](#2-1-프로덕션-스택-실측-docker--postgresql--실-llm-2026-07-13-quick) 참고.
동시성 50 실측은 비용/레이트리밋 부담으로 보류, 파일럿 계약 확정 후 진행 예정.
...
## 2-1. 프로덕션 스택 실측 (Docker + PostgreSQL + 실 LLM, 2026-07-13, quick)

`docker compose up -d --build`로 기동한 실제 스택(api/worker/db=Postgres 16/redis,
`DB_POOL_SIZE=30`) 대상, 실 Gemini + Pinecone 키로 측정. 비용/레이트리밋 부담으로
quick 모드(동시성 10, 시나리오당 8초)만 진행 — 동시성 50은 파일럿 계약 후 별도 실측.

| 시나리오 | RPS | p50 | p95 | p99 | 에러율 |
|---|---|---|---|---|---|
| baseline (GET /) | 1,186 | 5ms | 26ms | 46ms | 0% |
| 사용량 조회 (GET /usage) | 146 | 68ms | 79ms | 93ms | 0% |
| 위젯 Q&A (실 LLM 포함) | 9.9 | 826ms | 2,397ms | 2,617ms | 0% |
| 파트너 Q&A (실 LLM 포함) | 10.8 | 876ms | 1,141ms | 1,185ms | 0% |
| 원고 접수 2회차 (202 응답) | 84 | 116ms | 132ms | 317ms | 0% |

해석:
- API 계층 오버헤드(baseline/사용량조회/원고접수 202)는 하네스 측정치와 같은
  자릿수로 재현됨 — SQLite 스텁 측정이 과도하게 낙관적이지 않았음을 확인.
- Q&A p95(실 LLM 포함)는 1.1~2.4초로, §4 SLA 초안의 "Q&A 체감 응답 p95 < 8s"
  목표를 여유 있게 충족. 단, 시드 데이터가 Pinecone에 인덱싱되지 않은 신규
  원고라 "찾을 수 없음" 응답 경로였음 — 실제 인덱싱된 원고 기준 재측정 권장.
- 원고 접수(202) 응답 자체는 API 계층만 측정(실제 회차 분석은 Celery 워커가
  비동기 처리, 이번 측정 범위 밖).

재현: `scripts/loadtest/seed_prod.py`를 `docker compose exec api python -m
scripts.loadtest.seed_prod`로 실행해 실제 파트너/API키/위젯토큰을 시드한 뒤
`docker cp`로 `seed.json`을 호스트로 꺼내고 `python scripts/loadtest/run_loadtest.py --quick`.
...
`scripts/loadtest/seed_prod.py`가 이 시딩을 자동화한다 (§2-1 참고).
