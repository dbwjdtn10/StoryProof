...
이건 그다지 놀라운 일이 아니었어요. 앨리스는 토끼가 혼잣말로 "이런, 이런! 늦겠어!"라고 말하는 걸 들었을 때도 그게 그렇게 이상하다고 생각하지 않았어요. (나중에 생각해 보니 이상하게 여겼어야 했지만, 그때는 모든 게 너무나 자연스러워 보였거든요.) 하지만 토끼가 실제로 조끼 주머니에서 시계를 꺼내 보더니 서둘러 달려가는 걸 보자, 앨리스는 벌떡 일어났어요. 조끼 주머니가 달린 토끼도, 거기서 꺼낼 시계를 가진 토끼도 본 적이 없다는 생각이 번뜩 스쳤거든요. 호기심에 불타 앨리스는 들판을 가로질러 토끼를 쫓아갔고, 토끼가 울타리 밑 큰 토끼 굴로 뛰어들어 가는 걸 보았어요.`);
    ...
    const handleNavigateToQuote = (quote: string) => {
        // 1. Try to find the scene containing the quote
        const sceneIndex = sceneTexts.findIndex(text => text.includes(quote));

        if (sceneIndex !== -1) {
            scrollToScene(sceneIndex);

            // Optional: You could add logic here to highlight the specific text within the textarea
            // But scrolling to the scene is the primary requirement
        } else {
            // Backup: Try to find approximate match (e.g. first 20 chars) if exact match fails
            // This handles cases where user might have slightly edited text after analysis
            const shortQuote = quote.substring(0, 20);
            const approxIndex = sceneTexts.findIndex(text => text.includes(shortQuote));

            if (approxIndex !== -1) {
                scrollToScene(approxIndex);
            } else {
                alert("해당 문장을 현재 텍스트에서 찾을 수 없습니다.");
            }
        }
    };
