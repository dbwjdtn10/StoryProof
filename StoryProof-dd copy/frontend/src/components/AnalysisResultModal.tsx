// frontend/src/components/AnalysisResultModal.tsx
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ padding: '24px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', margin: 0, display: 'flex', alignItems: 'center', gap: '8px', color: '#1f2937' }}>
                        <ShieldAlert size={24} color={isLoading ? '#4F46E5' : (result?.status === "분석 오류" ? "#9CA3AF" : (result?.status === "설정 파괴 감지" ? "#EF4444" : "#10B981"))} className={isLoading ? 'animate-pulse' : ''} />
                        설정 파괴 탐지기
                    </h2>
                    <button
                        onClick={onClose}
                        ...
                        <div>
                            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
                                ...
                                <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#1f2937', margin: 0 }}>
                                    {result.status === "설정 파괴 감지" ? "설정 오류가 감지되었습니다" :
                                        result.status === "분석 오류" ? "분석 중 오류가 발생했습니다" : "설정이 완벽합니다!"}
                                </h2>
                                {result.status === "분석 오류" && (
                                    <p style={{ color: '#6B7280', marginTop: '8px' }}>{result.message || '알 수 없는 오류가 발생했습니다.'}</p>
                                )}
                            </div>

                            {/* New structured format */}
                            {result.results && result.results.map((issue: AnalysisIssue, idx: number) => (
                                <div key={idx} style={{
                            ...
                            {result.status !== "설정 파괴 감지" && result.status !== "분석 오류" && (
